/* Renders the printable catalogue on the home page. */
(function () {
  var el = AP.el;

  /* Line-art thumbnails, drawn rather than shipped as images. */
  var ART = {
  ...
  function init() {
    ...
    AP.CATEGORIES.forEach(function (cat) {
      var items = AP.byCategory(cat.id);
      if (!items.length) return;
      root.appendChild(el('section', { class: 'section', id: cat.id }, [
        el('div', { class: 'section-head' }, [
          el('h2', { text: cat.label }),
          el('p', { text: cat.blurb })
        ]),
